...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Rate limit: max 3 per 60 seconds
    const allowed = await checkRateLimit(supabase, user.id, "client_wallet_purchase", 60, 3);
    if (!allowed) {
    ...
    // Get profit margin from api_config
    let marginPct = 35;
    ...
    // Client pays retail price with profit margin applied
    const apiCost = interpolateApiCost(credits, priceTable);
    const totalPrice = Math.ceil(apiCost * (1 + marginPct / 100));
    ...
    if (!debited) {
      ...
      return respond({ error: "Saldo insuficiente", code: "INSUFFICIENT_BALANCE" }, 400);
    }
    ...
    console.error("client-wallet-purchase error:", err);
